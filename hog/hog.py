...
def perfect_square(x):
    square_root_x = x**0.5
    if square_root_x %1 ==0:
        return True 
    else:
        return False
def next_perfect_square(x):
    if perfect_square(x) == True:
        square_root = x ** 0.5 
...
def is_always_roll(strategy, goal=GOAL):
    ...
    c_tracker=0
    t_tracker=0
    while t_tracker<goal:
        tracker=strategy(c_tracker,t_tracker)
        while c_tracker<goal:
            tracker=strategy(c_tracker,t_tracker)
            if tracker!=roller:
                return False
            c_tracker+=1
...
def make_averaged(original_function, total_samples=1000):
    ...
    def average_value(*args):
        ...
        while times!=total_samples:
            total_func=original_function(*args)
            total+=total_func
            times+=1
        average=total/times
        return average
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    largest_roll=0
    while roll<=10:
        average_curr = make_averaged(roll_dice, total_samples)(roll,dice)
        if average_curr>largest_total:
            largest_total, largest_roll = average_curr, roll
        roll+=1
    return largest_roll
