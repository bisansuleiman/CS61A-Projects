...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant) == False and place.ant.can_contain(self) == False:
    ...
    def double(self):
        ...
        if self.was_doubled == False:
            self.damage = self.damage *2 #ov
            self.was_doubled = True
        if self.is_container==True and self.ant_contained != None:
            if self.ant_contained.was_doubled == False:
                self.ant_contained.damage = self.ant_contained.damage *2
                self.ant_contained.was_doubled = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        #         return None
        #     return self.place.entrance.nearest_bee()
        
        k=self.place
        counter = 0
        # while self.lower_bound<=counter<=self.upper_bound:
        while k!=None and k.is_hive== False:
            if k.bees!=[] and self.lower_bound<=counter<=self.upper_bound:
                return random_bee(k.bees)
            else:
                k=k.entrance
                counter +=1
        

        # elif type(self.place.entrance) != Hive:
        #     if self.place.entrance.bees>0:
        #         return random_bee(self.place.entrance.bees)
        #     else:
        #         return self.place.entrance.nearest_bee(self.place.entrance)
        # else:
        #     return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # b_list = self.place.
        # for bee in curr_place:
        #     if self.health - amount <= 0:

        # super().reduce_health(self,amount+self.damage)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # check if there is bee
        if bee !=None:
            """reduce bee health to ZERO and set turns_to_chew to 3"""
            bee.reduce_health(bee.health) #damage should be equal to bee current health
            self.turns_to_chew = self.chewing_turns

    def action(self, gamestate):
        """modify turns_to_chew with every turn"""
        if self.turns_to_chew == 0:
            """reduce bee health to ZERO and set turns_to_chew to 3"""
            #random_bee(self.place.bee).reduce_health(1) #damage should be equal to bee current health
            #self.turns_to_chew = 3
            self.eat_bee(random_bee(self.place.bees))
            
        else:
            """decrement turns_to_chew if it is chewing and do nothing"""
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        """Does 1 damage to all bees in its place AND protects the contained ant"""
        bee_copied = list(self.place.bees)
        super().action(gamestate)
        if bee_copied != None:
            for bee in bee_copied:
                bee.reduce_health(self.damage)
            # elif bee.health==0:
            #     bee.remove_from(self.place)
        
        # and allow contained ant to perform its action
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        p = self.place.exit 
        while p:
            if p.ant: # get error that NoneType has no attribute was_doubled
                # if p.ant.was_doubled==False:
                p.ant.double()
